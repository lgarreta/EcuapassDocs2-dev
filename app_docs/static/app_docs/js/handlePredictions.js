...
function handleFocus (event, textarea, mainFields) {
	...
	console.log (">>> txtId:", originalTxtId)
	...
	for (let i=2; i < numId; i++) {
        ...
        if (i < 10) // All main fields including "txt02"
		    valuesInputs [loopTxtId] = document.getElementById (loopTxtId).value;
	}

	console.log (">>> Values:", valuesInputs)
	const csrftoken = getCookie('csrftoken');
	// Call view predict function
	$.ajax({
		type        : 'POST',
		url         : '/cartaporte/prediccion/' + originalTxtId,
		data        : JSON.stringify ({'txtId': originalTxtId, 'inputsTextValues': valuesInputs}),
