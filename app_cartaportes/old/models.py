...
class Cartaporte (EcuapassDoc):
	...
	def setValues (self, cartaporteDoc, procedimiento, nombreUsuario, azureValues):
		self.numero        = cartaporteDoc.numero
		self.documento     = cartaporteDoc
		self.procedimiento = procedimiento
		self.usuario       = self.getUserByUsername (nombreUsuario)
		# Get subject instance for 'remitente' and 'destinatario'
		self.remitente     = self.getSubjectInstance ("02_Remitente", azureValues)
		self.destinatario  = self.getSubjectInstance ("03_Destinatario", azureValues)
		# Get 'fecha recepcion'
		self.fecha_emision = self.getFechaRecepcion ("31_FechaRecepcion", azureValues)
		
	#-- Get fecha de recepcion
	def getFechaRecepcion (self, fieldName, azureValues):
		jsonFieldsPath, runningDir = self.createTemporalJson (azureValues)
	...
	def getSubjectInstance (self, subjectType, azureValues):
		info = None
		try:
			jsonFieldsPath, runningDir = self.createTemporalJson (azureValues)
			cartaporteInfo    = CartaporteByza (jsonFieldsPath, runningDir)
	...
	def createTemporalJson (self, azureValues):
		tmpPath        = tempfile.gettempdir ()
		jsonFieldsPath = os.path.join (tmpPath, f"CARTAPORTE-{self.numero}.json")
		json.dump (azureValues, open (jsonFieldsPath, "w"))
